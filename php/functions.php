...
function get_publish_works()
{
  ...
  {
    echo "{$sql} Syntax execution failed. Error message:" . mysqli_error($_SESSION['link']);
  }
 ...
 * 更新作品
 */
function update_work($id, $title,$intro,$content,$travel_tips1,$travel_tips2, 
$image_path, $image_path_s1,$image_path_s2, $publish)
{
	//宣告要回傳的結果
  $result = null;

  $title = isset($title) ? htmlspecialchars($title) : '';
  $intro = isset($intro) ? htmlspecialchars($intro) : '';
  $content = isset($content) ? htmlspecialchars($content) : '';
  $travel_tips1 = isset($travel_tips1) ? htmlspecialchars($travel_tips1) : '';
  $travel_tips2 = isset($travel_tips2) ? htmlspecialchars($travel_tips2) : '';
  ...
  $image_path_s1_query = "`image_path_s1` = '{$image_path_s1}'";
	if($image_path_s1 == '')
	{
		$image_path_s1_query = "`image_path_s1` = NULL";
	}
  $image_path_s2_query = "`image_path_s2` = '{$image_path_s2}'";
	if($image_path_s2 == '')
	{
		$image_path_s2_query = "`image_path_s2` = NULL";
	}

	//更新語法
  $sql = "UPDATE `works` SET `title`='{$title}', `intro`='{$intro}', `content`='{$content}', `travel_tips1`='{$travel_tips1}', `travel_tips2`='{$travel_tips2}', {$image_path_query}, {$image_path_s1_query}, {$image_path_s2_query}, `publish`={$publish} WHERE `id`={$id};";
 ...
 * 新增作品
 */
function add_work($title,$intro,$content,$travel_tips1,$travel_tips2, $image_path, $image_path_s1,$image_path_s2,$publish)
{
	//宣告要回傳的結果
  $result = null;
  ...
  $travel_tip1 = htmlspecialchars($travel_tips1);
  $travel_tip2 = htmlspecialchars($travel_tips2);
	//建立者id
  ...
  //處理圖片路徑
  $image_path_value = $image_path ? "'{$image_path}'" : 'NULL';
  $image_path_s1_value = $image_path_s1 ? "'{$image_path_s1}'" : 'NULL';
  $image_path_s2_value = $image_path_s2 ? "'{$image_path_s2}'" : 'NULL';


	//新增語法
  $sql = "INSERT INTO `works` (`title`, `intro`, `content`, `travel_tips1`, `travel_tips2`,
        `image_path`, `image_path_s1`, `image_path_s2`, `publish`, `upload_date`, `create_user_id`)
        VALUES ('{$title}', '{$intro}', '{$content}', '{$travel_tips1}', '{$travel_tips2}', {$image_path_value}, 
        {$image_path_s1_value}, {$image_path_s2_value}, {$publish}, '{$upload_date}', '{$create_user_id}');";
