...
<html lang="zh-TW">
  ...
  <body>
    ...
    <!-- 網站內容 -->
	<div class="head_out">
          <div class="head">Add Account</div>
    </div>
    <div class="content">
      <div class="container">
        ...
        <div class="row">
          ...
          <div class="col-xs-12">
            <form class="register" style="font-size: 20px;"><!-- 沒有設定  method 跟 action 交給之後的 ajax 處理-->
              <div class="form-group">
                <label for="username">Username</label>
                <input style="height:45px; font-size: 16px;" type="text" class="form-control" id="username" name="username" placeholder="User Name" required>
              </div>
              <div class="form-group">
                <label for="password">Password</label>
                <input style="height:45px; font-size: 16px;" type="password" class="form-control" id="password" name="password" placeholder="Password" required>
              </div>
              <div class="form-group">
                <label for="confirm_password">Confirm_Password</label>
                <input style="height:45px; font-size: 16px;" type="password" class="form-control" id="confirm_password" name="password" placeholder="Re_Password" required>
              </div>
              <div class="form-group">
                <label for="name">Name</label>
                <input style="height:45px; font-size: 16px;" type="text" class="form-control" id="name" name="name" placeholder="Name" required>
              </div>
							<div style="text-align: center;">
								<button type="submit" class="btn btn-default" style="font-size: 16px; margin-top: 20px;">
                	register
              	</button>
							</div>
      	...
      	$("#username").on("keyup", function(){
      		...
      		if(keyin_value != '')
      		{
	      		...
	      		$.ajax({
			        ...
			        {
			        	alert("The account is duplicated. Registration is not allowed.");
      		...
      		else
      		{
	        	...
	        	//若密碼都不一樣就警告。
	          alert("The two entered passwords do not match. Please verify.");
	        ...
	        else
	        {
	        	//若當密碼正確無誤，就使用 ajax 送出
	      		$.ajax({
			        ...
			        if(data == "yes")
			        {
			          alert("Insertion successful. Click OK to return to the list.");
			        	//註冊新增成功，轉跳到登入頁面。
			        	window.location.href="member_list.php";
			        }
			        else
			        {
			        	alert("Registration failed. Please contact the system administrator for assistance.");
			        	$("div.loading").html('');
			        }
			        
			      }).fail(function(jqXHR, textStatus, errorThrown) {
			      	//失敗的時候
			      	alert("An error occurred. Please check the console log for details.");
			        console.log(jqXHR.responseText);
			      });
